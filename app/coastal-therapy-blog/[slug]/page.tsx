...
const posts: Record<string, {
  ...
  inlineImages?: { src: string; alt: string; caption?: string }[]
}> = {
  'speech-therapy-introducing-the-g-sound': {
    ...
    excerpt: 'Replacing /d/ for /g/ is called a phonological pattern called "fronting." It\'s typical for young kids to substitute sounds, but once past a certain age, we expect them to produce the sound naturally.',
...
If you're concerned about your child's speech development, our team at Coastal Pediatric Therapy Center is here to help. We serve families in Jacksonville Beach and Mandarin, FL.`,
  ...
  '5-easy-ways-to-incorporate-sensory-play': {
    ...
    inlineImages: [
      { src: '/images/blog/sensory-play-1.jpeg', alt: 'Child playing with sensory bin at Coastal Pediatric Therapy Center' },
      { src: '/images/blog/sensory-play-2.jpeg', alt: 'Sensory play activity for kids' },
      { src: '/images/blog/sensory-play-3.jpeg', alt: 'Child engaged in sensory play' },
    ],
    excerpt: 'Sensory play can seem daunting, but it doesn\'t have to be complicated. Here are 5 simple ways to incorporate it into your child\'s routine using things you already have at home.',
    content: `5 Easy Ways to Incorporate Sensory Play

Today's topic is for parents who want to add sensory play to their child's playtime. We always hear how beneficial sensory play can be, but it can seem daunting to think of how to expose our children to this awesome play concept. Here are 5 simple ways to incorporate sensory play.
...
If your child has difficulty tolerating sensory input or seeks excessive sensory stimulation, our occupational therapists can help. Contact Coastal Pediatric Therapy Center at (904) 372-4070.`,
  ...
  'physical-therapy-for-torticollis': {
    ...
    inlineImages: [
      { src: '/images/blog/torticollis-1.jpg', alt: 'Baby with torticollis receiving physical therapy' },
      { src: '/images/blog/torticollis-2.png', alt: 'Pediatric physical therapy for torticollis at Coastal Therapy' },
      { src: '/images/blog/torticollis-3.png', alt: 'Physical therapy treatment for infant torticollis' },
    ],
...
Torticollis can affect the baby's whole body. Head and neck positioning can also contribute to:
...
If you notice your baby consistently turning their head to one side, contact your pediatrician and ask for a physical therapy referral. The team at Coastal Pediatric Therapy Center in Jacksonville Beach and Mandarin, FL is experienced in treating infants and young children with torticollis.`,
  ...
  'world-down-syndrome-day': {
    ...
    inlineImages: [
      { src: '/images/blog/down-syndrome-1.jpg', alt: 'Coastal Pediatric Therapy Center celebrating World Down Syndrome Day', caption: 'Our Physical Therapist Karli and her sister Kaci' },
      { src: '/images/blog/down-syndrome-2.jpg', alt: 'World Down Syndrome Day celebration at Coastal Therapy' },
      { src: '/images/blog/down-syndrome-3.jpg', alt: 'Celebrating Down Syndrome awareness at Coastal Pediatric Therapy' },
    ],
    excerpt: 'Every year on March 21st we celebrate World Down Syndrome Day to honor the uniqueness of the triplication of the 21st chromosome, which is the genetic cause of Down Syndrome.',
...
Once Sophia was diagnosed, we pushed for the doctors to look more closely at my oldest niece Ava, who was having visual abnormalities as well. If it had not been for Sophia's early diagnosis we might not have had the opportunity and knowledge for early intervention with Ava.
...
Contact Coastal Pediatric Therapy Center at (904) 372-4070 to schedule an evaluation in Jacksonville Beach or Mandarin, FL.`,
  ...
  'is-it-ok-if-my-baby-always-holds-their-head-tilted-to-the-same-side': {
    ...
    excerpt: 'Holding a head tilted to the same side or always looking in one direction could be a sign your child has Torticollis. Early physical therapy can help.',
...
Torticollis is the inability to straighten or turn the head to each side equally. Torticollis is the third most common congenital musculoskeletal anomaly in children and will generally be present in the first week to 10 days after birth, although most people notice this in their baby or child in the first two months.
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    image: post.featuredImage ? `https://coastaltherapy.net${post.featuredImage}` : undefined,
    author: {
      '@type': 'Organization',
    ...
    <>
      ...
      <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
        ...
        <article style={{ maxWidth: '820px', margin: '0 auto', padding: '0 1.5rem 3rem' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '2rem 2.5rem' }}>
            ...
            {paragraphs.map((block, i) => {
              const isHeading = !block.startsWith('-') && block.length < 80 && !block.includes('.') && !block.includes(':')
              const isList = block.split('\n').every(l => l.trim().startsWith('-'))

              const el = isList ? (
                <ul key={i} style={{ margin: '0 0 1.25rem 1.25rem', padding: 0 }}>
                  {block.split('\n').filter(Boolean).map((item, j) => (
                    <li key={j} style={{ fontSize: '1rem', color: '#374151', lineHeight: 1.8, marginBottom: '0.4rem' }}>{item.replace(/^-\s*/, '')}</li>
                  ))}
                </ul>
              ) : isHeading ? (
                <h2 key={i} style={{ fontSize: '1.3rem', fontWeight: 700, color: '#1e3a5f', margin: '2rem 0 0.75rem', paddingTop: i > 0 ? '0.5rem' : 0, borderTop: i > 0 ? '1px solid #f1f5f9' : 'none' }}>{block}</h2>
              ) : (
                <p key={i} style={{ fontSize: '1rem', color: '#374151', lineHeight: 1.85, marginBottom: '1.25rem' }}>{block}</p>
              )

              // Insert inline images after midpoint
              if (post.inlineImages && i === midpoint && post.inlineImages.length > 0) {
                return (
                  <div key={`img-block-${i}`}>
                    <div style={{ display: 'flex', gap: '1rem', margin: '1.5rem 0', flexWrap: 'wrap', justifyContent: 'center' }}>
                      {post.inlineImages.map((img, j) => (
                        <div key={j} style={{ flex: '1 1 200px', maxWidth: '260px', textAlign: 'center' }}>
                          <div style={{ position: 'relative', width: '100%', paddingBottom: '130%', borderRadius: '8px', overflow: 'hidden', backgroundColor: '#f1f5f9' }}>
                            <Image src={img.src} alt={img.alt} fill style={{ objectFit: 'cover' }} />
                          </div>
                          {img.caption && <p style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.4rem', fontStyle: 'italic' }}>{img.caption}</p>}
                        </div>
                      ))}
                    </div>
                    {el}
        ...
        <section style={{ backgroundColor: '#1e7faa', color: '#fff', padding: '3rem 1.5rem', textAlign: 'center' }}>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '0.75rem' }}>Ready to get your child evaluated?</h2>
