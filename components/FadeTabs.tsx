  ...
  return (
    <div className="relative overflow-hidden md:overflow-visible">
      ...
      <div className="tab-fade-inner md:overflow-x-visible md:w-full" ref={scrollRef}>
        <div className="flex gap-1 py-2.5 min-w-max md:min-w-0 md:w-full md:justify-center px-1">
          {tabs.map((tab, i) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                key={tab.id}
                onClick={() => onChange(i)}
                className={`flex items-center justify-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold transition-all whitespace-nowrap md:flex-none ${
                  isActive
                    ? isPurple
