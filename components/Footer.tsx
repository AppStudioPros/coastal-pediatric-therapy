    ...
    <footer className="bg-[#1e3a4a] text-white">
      <div className="max-w-6xl mx-auto px-4 py-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div>
          ...
          {subStatus === "sent" ? (
            <p className="text-[#FFD940] text-sm font-semibold">Thanks! You&apos;re subscribed. ✓</p>
          ) : (
            <form onSubmit={handleSubscribe} className="space-y-3">
              <input
                ...
                className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2.5 text-sm text-white placeholder-white/40 focus:outline-none focus:border-[#FFD940] transition"
              />
              <button
                type="submit"
                className="w-full cta-pulse-y4 bg-[#FFD940] text-[#1e3a4a] font-bold py-2.5 rounded-lg text-sm hover:bg-[#FFEB70] transition"
              >
                Subscribe
              </button>
              {subStatus === "error" && (
